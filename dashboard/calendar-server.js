#!/usr/bin/env node
/**
 * Dev-only server: serves /calendar-api/* for the dashboard (proxied by Vite).
 * Run with: npm run dev:calendar (or use npm run dev which starts this + Vite).
 * Uses SERVICE_ACCOUNT.json; calendars are stored in the DB (same as server.js).
 */
...
// Saved calendars (same routes as server.js)
...
app.listen(PORT, () => {
  console.log(`Calendar API: http://localhost:${PORT}/calendar-api (saved-calendars + events)`);
});
